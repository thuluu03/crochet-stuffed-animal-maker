...
export function SaveAndColor() {
  const { parts, selectedInstanceId, getPart, updatePart, removePart, buildPayload, setSelected } = useDesign();
      ...
      {selectedPart && (
        <div className="color-panel">
          ...
          <div className="row-colors">
            ...
            <p className="segment-hint">Set a color per horizontal segment (row).</p>
            {Array.from({ length: segmentCount }, (_, i) => (
              <div key={i} className="color-row">
                ...
                />
                <span className="color-hex">{selectedPart.rowColors?.[i] ?? selectedPart.color}</span>
              </div>
            ))}
          </div>
          <button
